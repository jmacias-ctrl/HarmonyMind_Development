...
const informacion_experto: React.FC<UserDetailPageProps> = ({ match }) => {
    ...
    const fetch_posts = () => {
        if (isLoading == true) {
            fetch(`http://192.168.1.6:8000/api/expert_connection/get_schedules?expert_id=${match.params.expertoId}`, {
                "method": "GET",
                "headers": {
                            ...
                            ></IonAlert>) : (<IonAlert
                                ...
                                buttons={[
                                    ...
                                    },
                                    {
                                        ...
                                        role: 'confirm',
                                        handler: () => {
                                            fetch(`http://192.168.1.6:8000/api/expert_connection/set_appointment?id_horario=${horaEscogida}`, {
                                                "method": "POST",
                                                "headers": {
                                                    'Accept': 'application/json',
                                                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
